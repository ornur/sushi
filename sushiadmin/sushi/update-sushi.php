<?php
    ...
    $id = $_GET["id"];
    $sushi = mysqli_fetch_assoc(mysqli_query($link, "SELECT * FROM `tbl_sushi` WHERE `id` = $id"));
    ...
    <body>
        ...
        <main style="margin-top: 20px;">
            <div class="container">
                ...
                <form action="save-sushi.php" method="post" enctype="multipart/form-data">
                    <input type="hidden" value="<?php echo $sushi["id"];?>" name="id"> 
                    <div class="col-md-6">
                        <label class="form-label" for="var-title">Новая имя еды</label>
                        <input type="text" name="name" class="form-control" value="<?php echo $sushi["name"]?>" id="var-title">
                    </div>

                    <div class="col-md-12" id="text-form">
                        <label for="text" class="form-label">Новая содержимое еды</label>
                        <input type="text" name="description" value="<?php echo $sushi["description"]?>" class="form-control" id="text">
                    </div>

                    <div class="col-md-12" id="text-form">
                        <label for="text" class="form-label">Новая цена еды</label>
                        <input type="text" name="price" class="form-control" value="<?php echo $sushi["price"]?>" id="text" pattern="[0-9]{3,7}">
                    </div>
